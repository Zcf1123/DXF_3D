...
- 如果 `model_understanding_hints[*].kind == "top_line_arc_plate_with_center_bore"`，这是 TOP 完整 LINE/ARC 外轮廓沿 Z 拉伸的异形薄板，中心圆为 Z 轴贯穿孔。必须使用 `construction.ordered_profile_edges` 在 XY 平面构造真实外轮廓：`LINE` 用 `Part.LineSegment(p0, p1).toShape()`，`ARC` 按边的 `p0/center/radius/p1/start_angle/end_angle/clockwise` 计算弧上中点并构造 `Part.Arc(...)`；再 `Part.Face(wire).extrude(App.Vector(0, 0, height_z))`。中心孔必须使用 `construction.bore` 或 `hole_hints` 切除。禁止用 `radius_hint`、中心圆或经验半径重新生成外轮廓；禁止把外轮廓缩小成圆柱、12 边形近似或只使用局部圆弧点。
- 如果 `model_understanding_hints[*].kind == "stacked_cylinder_hex_top_bore"`，这是同轴组合件，不是简单圆柱：下部长圆柱杆、顶部短圆柱/圆盘、顶部六棱柱凸台共轴叠加，并有中心 Z 轴孔。必须按 `construction` 中的 `stem_radius/outer_radius/stem_z0/stem_z1/top_z0/top_z1/hex_vertices_2d/bore_radius` 分别建下部长圆柱、顶部短圆柱和六棱柱，逐个 `fuse`，最后用 Z 轴圆柱切中心孔。禁止把内部六边形写成“按提示简化”并省略，也禁止把整件建成单个圆柱。
