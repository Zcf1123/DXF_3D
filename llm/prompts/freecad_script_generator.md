...
- 如果 `model_understanding_hints[*].kind == "toothed_disk_from_top_profile"`，这是齿轮/带齿圆盘：TOP 的 `construction.outer_profile_points_2d` 是真实齿形外轮廓，必须用它在 XY 平面构造闭合轮廓并沿 Z 拉伸 `height_z`，再按 `bore_hole` 切中心孔；FRONT/LEFT 主要用于厚度校验，不要把它们的多个矩形投影建成阶梯块或连杆组件。
