...
把 `.dxf` 工程图（标准三视图：FRONT / TOP / LEFT）解析并重建为 FreeCAD 3D 实体，
...
├── view_classifier.py      # 把实体聚类到 front/top/left 三视图
...
List[DxfEntity]          ← LINE / CIRCLE / ARC / LWPOLYLINE / DIMENSION …
   │
   ▼  view_classifier.py
ViewBundle × 3           ← front (TL) / left (TR) / top (BL)
   │
...
按 TOP/XY 平面处理；标准 FRONT/TOP/LEFT 三视图输入保持原逻辑。该路径会跳过 LLM
视图重命名和特征改写，闭合轮廓直接拉伸，内部圆按贯穿孔处理，内部闭合线/弧轮廓
按异形贯穿孔处理，单圆外轮廓拉伸为圆柱。输出阶段仍生成模型三视图 PNG，`.FCStd`
中会补充由 3D 模型投影得到的 FRONT/LEFT 视图线框。
...
球体与通孔的区分依赖三视图联动：TOP 圆心为 `(X,Y)`，FRONT 为 `(X,Z)`，LEFT
为 `(Y,Z)`；三视图各只有一个同半径圆且 W/D/H 约等于直径时才输出 `sphere`。
单视图圆或有主体轮廓/隐藏线支撑的圆仍按通孔处理。
例外：命令行提供 `--extrude-depth` 且仅识别到单一视图时，单个圆会作为外轮廓拉伸为圆柱；
若有更大的外轮廓，内部圆仍作为贯穿孔。

圆筒类三视图若 TOP 只有同心圆，最大圆会作为圆柱外轮廓拉伸；内圆结合 FRONT/LEFT
隐藏线识别为孔。隐藏线若未覆盖完整高度，则按隐藏线轴向跨度生成盲孔（`blind=true`，
`through_length` 为实际切除深度），避免误切为贯穿孔。

上下圆盘中间圆柱等同轴组合件通过 `cylinder_stack` 表达：从 FRONT/LEFT 的阶梯侧视图
按 Z 高度带提取半径，要求两侧视图一致，并由 TOP 同心圆校验半径。建模时逐段生成圆柱
并 fuse，适合“下大圆盘 + 中间小圆柱 + 上大圆盘”及更多同轴台阶。

组合体：当 `--model-intent` 明确包含“组合/多个物体/连接/相接”等语义时，
`feature_inference.py` 可输出多个带 `additive_component=true` 的 `extrude_profile`。
`freecad_builder.py` 会将多个正实体按顺序 fuse 后再执行后续切除/倒角，适合
圆柱 + 棱柱 + 小圆柱这类简单多主体连接件。旧的单主体路径仍保持原逻辑。
