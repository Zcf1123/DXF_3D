...
- 典型对应：
  ...
  · 阶梯轴侧面：FRONT 是台阶外形 → `plane=XZ, source_view=front, depth=D`
   · L 形支座侧面：LEFT 是 L 形 → `plane=YZ, source_view=left, depth=W`
...
2. FRONT 或 LEFT 中至少一个视图存在真实 ARC，表示上下端面由圆弧过渡，
   不是普通直线倒角。
...
5. `distance` 必须来自 FRONT/LEFT 中短竖边相对上下端面的内缩量；不能凭
...
4. **不得创造草案中没有的特征**：不要凭空添加圆角、肋板、阵列孔或螺纹。
   ...
   TOP 视图的大同心圆、FRONT/LEFT 的上下圆弧通常表示 R 形端面倒角包络；
